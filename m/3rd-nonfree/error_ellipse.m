%>@ingroup graphicsapi
%>@file
%>@brief plot an error ellipse, or ellipsoid, defining confidence region
%>
%>@verbatim
...
%>      'clip' - specifies a clipping ruleRadii. Portions of the ellipse, -oid,
%>        outside the ruleRadii will not be shown.
%>
%>    NOTES: C must be positive definite for this function to work properly.
%>@endverbatim
%>
%>  Modified 04/Apr/2010 by JT
...
if length(varargin) >= 1 & ~ischar(varargin{1})
  irerror('Invalid parameter/value pair arguments.') 
end
...
if conf <= 0 | conf >= 1
  irerror('conf parameter must be in range 0 to 1, exclusive')
end

[r,c] = size(C);
if r ~= c | (r ~= 2 & r ~= 3)
  irerror(['Don''t know what to do with ',num2str(r),'x',num2str(c),' matrix'])
end
...
if r==3 & c==3
  ...
  if any(eig(C) <=0)
    irerror('The covariance matrix must be positive definite (it has non-positive eigenvalues)')
  end
...
elseif r==2 & c==2
  ...
  h1=plot(scale*(x0+k*x),scale*(y0+k*y), 'Color', prop.Color, 'LineWidth', scaled(prop.LineWidth));
  set(h1,'zdata',z+1)
  if nargout
    h=h1;
  end
else
  irerror('C (covaraince matrix) must be specified as a 2x2 or 3x3 matrix)')
end
%axis equal
...
%   x and y.
function [x,y,z] = getpoints(C,clipping_ruleRadii)
...
% Clip data to a bounding ruleRadii
if nargin >= 2
  r = sqrt(sum(xy.^2,2)); % Euclidian distance (distance from center)
  x(r > clipping_ruleRadii) = nan;
  y(r > clipping_ruleRadii) = nan;
  z(r > clipping_ruleRadii) = nan;
end
...
for ii=1:length(varargin)
  ...
  if isempty(TargetField)
    if ~ischar(arg)
      irerror('Propery names must be character strings');
    end
    f = find(strcmp(prop_names, arg));
    if length(f) == 0
      irerror('%s ',['invalid property ''',arg,'''; must be one of:'],prop_names{:});
    end
    TargetField = arg;
  else
...
if ~isempty(TargetField)
  irerror('Property names and values must be specified in pairs.');
end
