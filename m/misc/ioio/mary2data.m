...
%> @param height
%> @return A dataset
function [data, flag_error] = mary2data(wild, trimdot, flag_image, height)
...
if flag_image
    if no_files/height ~= floor(no_files/height)
        irerror('mary2data: Invalid image height!');
    end;
end;
...
flag_first = 1;
ipro = progress2_open('MARY2DATA', [], 0, no_files);
cnt_error = 0;
errors = {};
ii = 0;
for i = 1:no_files
